...
Each test case will contain a string having length len(S1)+len(S2), which will be concatenation of both the strings described above in the problem.
...
Here, S1 = "aaa" and S2 = "bbb". So the solution is to replace all character 'a' in string a with character 'b'.
...
He had to replace 'a' from S1 with 'b' so that S1 = "xbxb" and we can rearrange its letter to "bbxx" in order to get S2.
**/
...
int main() {
    ...
    while(t--){
        ...
        if(s.length() % 2 != 0){
            ...
            unsigned mid = s.length() / 2;
            for(unsigned i = 0; i < mid; i++){
                s1count[s[i] - 97]++;
            }
            unsigned nchanges = 0;
            for(unsigned i = mid; i < s.length(); i++){
                if(s1count[s[i]-97] > 0){ 
                    s1count[s[i]-97]--;
                } else {
                    nchanges++;
