...
unsigned digits(long unsigned n){
    unsigned base = 10;
    unsigned d = 0;
    do {
        d++;
...
int main() {
    ...
    cin >> p >> q;
    unsigned counter = 0;
    for(long unsigned n = p; n <= q; n++){
        unsigned d = digits(n);
        string ssquare = to_string(n*n);
        unsigned dd = digits(n*n);
        string sl = "";
        if (dd % 2 == 0)
            sl = ssquare.substr(0, d);
        else
            sl = ssquare.substr(0, d-1);
        reverse(ssquare.begin(),ssquare.end());
        string sr = ssquare.substr(0,d);
